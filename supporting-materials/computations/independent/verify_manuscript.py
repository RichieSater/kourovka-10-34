...
for forbidden in [
    ...
    r"\section{Verification and trust boundary}",
    "These are distinct proof architectures.",
    "certified inventory",
    "exception manifest",
    "successful record",
    "action fingerprint",
...
for token in [
    ...
    "Every required group and coordinate closure is checked once",
]:
    if token not in TEX:
...
required = [
    ...
    r"\subsection*{Outline of the proof}",
    ...
    "Stable product-supplement criterion",
    ...
    r"\section{Finite and sporadic computations}\label{app:finite}",
    r"\section{Supplementary formal verification}\label{app:trust}",
    ...
    "For $\\PSL(3,2)$ with $x=2$, the computation uses",
    "normal saturation was checked by three different\novergroup and embedding-orbit calculations",
    "These calculations use the same\nGAP installation, group representations",
    "Altogether this gives $42$ selected",
    ...
    "do not prove the\nuniversally quantified family statements",
    "do not constitute an end-to-end formal proof",
    "No single proof assistant checks the translation",
    "constructs the coordinate factors and the faithful homomorphism",
    "$\\Aut(S^k)\\to\\Aut(S)\\wr S_k$",
    "GAP performs the finite calculations",
    ...
    "Their argument differs from ours",
    "arXiv:2608.19478v1",
    r"\cite{LPS1990}",
